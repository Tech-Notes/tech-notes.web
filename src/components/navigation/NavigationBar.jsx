...
const profile = {
  username: "Thazin Naing",
  image:
    "bg-[url('https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=900&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8cGVyc29ufGVufDB8fDB8fHww')]",
};
...
const NavigationBar = () => {
  ...
  return (
    <Div className="flex items-center justify-between pr-4 border-b border-gray-200 dark:border-gray-600 h-16 cursor-pointer relative">
      <Div
        className="flex items-center w-1/4 justify-center"
      ...
      <Div className="flex items-center gap-2">
        ...
        {token ? (
          <>
            <Div
              className={`w-7 h-7 bg-no-repeat bg-center bg-cover bg-fit rounded-2xl ${profile.image}`}
            ></Div>
              ...
              {!showArrow ? (
                <>
                  ...
                  <Div>
                    <Button
                      onClick={Logout}
                      className="absolute bg-light dark:bg-black top-16 py-2 px-4 right-4"
                    >
                      Log Out
                    </Button>
                  </Div>
